...
;			When SW2 is pressed, all LEDs light up.
;
...
.def ring = r16
.def break_flag = r20
.def init_turn_left = r21
.def init_turn_right = r23
...
check_break:
cpi break_flag, 0xFF ;check if breaking
brne check_right
rcall break_stop

check_right:
cpi right_turn_flag, 0xFF ;check if right turn is active and if ring should go
brne check_left
rcall right_turn

check_left:
cpi left_turn_flag, 0xFF ;check if left turn is active and if ring should go
brne check_none
rcall left_turn

check_none: ; nothing was pressed, output normal LED state
ldi output_leds, normal_state
...
and output_leds, init_turn_left ; make sure that right side LEDs are on
rjmp end
...
com ring
mov output_leds, ring ; copy currently on ring counter LED to output_leds
and output_leds, init_turn_right ; make sure that left side LEDs are on
rjmp end

;>>>>>>>>>>>>>>>>>>>>Break>>>>>>>>>>>>>>>>>>>>>>>>>>>
break_stop:
ldi output_leds, 0x00
rjmp end
...
ldi ring, 0b1110_1111
ldi init_turn_right, 0b1100_0000 ; make sure that if break is not on, reset normal state LEDs
com init_turn_right
...
ldi ring, 0b1111_0111
ldi init_turn_left, 0b0000_0011 ; make sure that if break is not on, reset normal state LEDs
com init_turn_left
...
interrupt_break:
rcall avoid_bouncing
com break_flag ;update break flag
cpi break_flag, 0xFF ;check if it's breaking. if not, reset all values.
brne reset_break

is_right:
cpi right_turn_flag, 0xFF ;if it's breaking, check if it is also showing right turn
brne is_left
ldi init_turn_right, 0b1111_0000
com init_turn_right
rjmp end_break

is_left:
cpi left_turn_flag, 0xFF ; when breaking, check if it's also showing left turn
brne end_break
ldi init_turn_left, 0b0000_1111
com init_turn_left
rjmp end_break

reset_break:
ldi init_turn_right, 0b1100_0000
com init_turn_right
ldi init_turn_left, 0b0000_0011
com init_turn_left
end_break:
reti
